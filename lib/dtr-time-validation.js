 ...
 * @param {string} sessions.otIn
 * @param {string} sessions.otOut
 * @returns {number} Total hours rounded to 2 decimal places
 */
export function calculateTotalHours({ amIn, amOut, pmIn, pmOut, otIn, otOut }) {
  const getSessionHours = (timeIn, timeOut, floorMinutes = null) => {
    let startMinutes = toMinutes(timeIn);
    const endMinutes = toMinutes(timeOut);

    if (startMinutes === null || endMinutes === null) return 0;

    if (floorMinutes !== null) {
      startMinutes = Math.max(startMinutes, floorMinutes);
    }

    return Math.max(0, (endMinutes - startMinutes) / 60);
  };

  const amFloor = toMinutes("08:00");
  const amHours = getSessionHours(amIn, amOut, amFloor);
  const pmHours = getSessionHours(pmIn, pmOut);
  const otHours = getSessionHours(otIn, otOut);
  ...
  if (
    ...
    const effectiveStart = Math.max(amInMinutes, amFloor);
    rawHours = Math.max(0, (pmOutMinutes - effectiveStart) / 60);
    // Auto-deduct lunch if shift spans 12:00-13:00
    if (effectiveStart < 12 * 60 && pmOutMinutes > 13 * 60) {
  ...
  if (!hasCompleteAM && hasCompletePM) {
    ...
    const effectiveStart = Math.max(amInMinutes, amFloor);
    if (effectiveStart < 12 * 60 && amOutMinutes > 13 * 60) {
      rawHours = Math.max(0, rawHours - 1);
    }
  }

  return Number((rawHours + otHours).toFixed(2));
}
